...
import { statusReportAdminOptions, typeReportOptions } from '@/constants/init-data';
import useBoolean from '@/hooks/useBoolean';
import { IReport, IReportFilterByAdmin, ReportPriority, ReportStatus, ReportType } from '@/interfaces/report';
import {
    formatCurrency,
    formatDate,
    formatDateTime,
...
import { getAllReportsByAdmin } from '@/services/report-service';
...
const ReportsTable = () => {
    const [form] = useForm<IReportFilterByAdmin>();
    ...
    const fetchReports = useCallback(async (data: IReportFilterByAdmin) => {
        setLoading(true);

        try {
            const reports = await getAllReportsByAdmin(data);
    ...
    const handleFilter = (values: IReportFilterByAdmin) => {
        fetchReports(values);
    };
    ...
    return (
        <>
            ...
            {activeFilter && (
                <TableFilter<IReportFilterByAdmin> onReset={handleReset} form={form} onFinish={handleFilter}>
                    <Col span={12}>
                        <Form.Item name="status" label="Trạng thái">
                            <Select options={statusReportAdminOptions} placeholder="Chọn trạng thái" />
                        </Form.Item>
                    </Col>
                    <Col span={12}>
